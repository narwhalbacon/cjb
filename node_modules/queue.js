...
function transcodeSong(uuid) {
	...
	var time0 = new RegExp('\\btime=(\\d\\d):(\\d\\d):(\\d\\d)\.(\\d\\d)\\b'); // new time format, 00:00:00.00
	var time1 = new RegExp('\\btime=([\\d\\.]+)\\b'); // old time format, in milliseconds
	...
	ffmpeg.stderr.on('data', function(data) {
		...
		match = time0.exec(str);
		if(match) {
			song.length = (match[1]*(60*60*1000))+(match[2]*(60*1000))+(match[3]*1000)+(match[4]*10);
		} else {
			match = time1.exec(str);
			if(match) { 
				song.length = match[1]*1000;
			}
		}
	});
